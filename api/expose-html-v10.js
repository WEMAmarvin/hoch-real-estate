...
export default async function handler(req, res) {
  try {
    ...
    ].filter(([,v]) => v !== null && v !== undefined && String(v).trim() !== "").slice(0, 9);

    const highlights = String(obj.highlights || "").split(/\n|•/).map(v => v.replace(/^-/, "").trim()).filter(Boolean).slice(0, 6);
    const descriptionLength = String(obj.beschreibung || "").length;
    const descClass = descriptionLength > 1300 ? "long" : (descriptionLength > 900 ? "medium" : "short");
    const images = (obj.bilder || []).slice(0, 12);
    const galleryCount = Math.min(images.length, 6);
    const secondImages = images.slice(0, 6);
...
<title>${esc(obj.titel)} – Exposé V10</title>
<style>
:root{
  --petrol:#00424a;
  --petrol2:#073f43;
...
.cover-object{position:absolute;top:640px;left:112px;width:calc(100% - 224px);text-align:center;color:#fff;font-size:21px;line-height:1.18;font-weight:900;text-transform:uppercase;letter-spacing:.018em;overflow:hidden;max-height:86px;text-wrap:balance}
/* Seite 2 */
...
.description-title{position:absolute;top:104px;left:74px;width:560px;color:#214D40;font-family:Georgia,'Times New Roman',serif;font-size:41px;line-height:1.03;font-weight:700;letter-spacing:-.02em;text-transform:uppercase;text-wrap:balance}
.description-copy{position:absolute;top:252px;left:74px;width:560px;max-height:360px;overflow:hidden;color:#202020;font-size:16px;line-height:1.32;font-weight:400}
.description-copy.medium{font-size:14.5px;line-height:1.25;max-height:395px}
.description-copy.long{font-size:13px;line-height:1.18;max-height:435px}
.highlights-title{position:absolute;top:634px;left:74px;width:560px;color:#202020;font-size:15px;line-height:1.2;font-weight:900}
.highlights-list{position:absolute;top:663px;left:93px;width:530px;margin:0;padding:0;color:#202020;font-size:13px;line-height:1.35;font-weight:400}
.highlights-list li{margin-bottom:5px}
.price-claim{position:absolute;left:74px;bottom:104px;color:#202020;font-weight:900;font-size:15px}
.price-claim:empty{display:none}
.price-box{position:absolute;left:74px;bottom:61px;display:flex;align-items:stretch;background:var(--petrol);color:#fff;height:43px;min-width:260px;max-width:560px;overflow:hidden}
.price-box .price-main{display:flex;align-items:center;padding:0 22px;font-size:19px;line-height:1.05;font-weight:400;white-space:nowrap}
.price-box .price-type{display:flex;align-items:center;padding:0 18px;border-left:1px solid rgba(255,255,255,.75);font-size:18px;line-height:1.05;font-weight:400;white-space:nowrap}
/* Seite 3 */
...
.facts-label{top:610px}
.object-images{position:absolute;left:74px;right:74px;top:134px;height:425px;display:grid;gap:8px}
...
.object-images.count-5{grid-template:1.25fr 1fr/1.35fr 1fr}
...
.facts-grid{position:absolute;top:645px;left:74px;width:calc(100% - 148px);display:grid;grid-template-columns:repeat(3,1fr);gap:10px 11px}
.fact-card{height:85px;border:1.5px solid var(--gold);background:rgba(247,244,239,.72);padding:13px 14px 10px;overflow:hidden}
.fact-label{color:var(--gray);font-size:8px;line-height:1;letter-spacing:.18em;font-weight:900;text-transform:uppercase;margin-bottom:12px}
.fact-value{color:#214D40;font-size:17px;line-height:1.1;font-weight:900;max-height:38px;overflow:hidden}
/* Seite 4 */
...
<body data-expose-template="v10-canva-intended-restored">
...
<section class="page bg-plain">
  ...
  <div class="description-title">${esc(obj.titel)}</div>
  <div class="description-copy ${descClass}">${nl2br(obj.beschreibung)}</div>
  ${highlightsHtml}
  <div class="price-claim">${esc(obj.claim || "")}</div>
...
<section class="page bg-plain">
  ...
  <div class="object-images count-${secondCount}">${secondGalleryHtml}</div>
